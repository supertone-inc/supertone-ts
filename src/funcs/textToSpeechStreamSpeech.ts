...
export enum StreamSpeechAcceptEnum {
 ...
 */
export function textToSpeechStreamSpeech(
  client: SupertoneCore,
  request: operations.StreamSpeechRequest,
  options?: RequestOptions & { acceptHeaderOverride?: StreamSpeechAcceptEnum },
): APIPromise<
  Result<
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.StreamSpeechRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON(
    "body",
    payload.APIConvertTextToSpeechUsingCharacterRequest,
    { explode: true },
  );
  ...
  const path = pathToFunc("/v1/text-to-speech/{voice_id}/stream")(pathParams);

  const headers = new Headers(compactMap({
    "Content-Type": "application/json",
    Accept: options?.acceptHeaderOverride
      || "application/x-ndjson;q=1, audio/mpeg;q=0.7, audio/wav;q=0",
  }));
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.stream(200, operations.StreamSpeechResponse$inboundSchema, {
      ctype: "audio/wav",
      hdrs: true,
      key: "Result",
    }),
    ...
    M.text(200, operations.StreamSpeechResponse$inboundSchema, {
