...
import { SupertoneDefaultError } from "../models/errors/supertonedefaulterror.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | SupertoneDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | SupertoneDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new SupertoneDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new SupertoneDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
