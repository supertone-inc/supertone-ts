...
export function applyPronunciationDictionary(
	...
	if (pronunciation_dictionary == null) return text;
	...
	for (let idx = 0; idx < pronunciation_dictionary.length; idx++) {
		...
		if (partial) {
			...
			if (!re.test(working)) {
				tokenToPronunciation.delete(token);
				continue;
			}
			// Reset lastIndex since we used .test with /g
			re.lastIndex = 0;
		...
		working = working.replace(re, `$1${token}`);
	}
